...
d = dict([(name, (a, e, t, orbit_class[t])) for (name, a, e, t) in Asteroids])
name_list = d.keys()
name_list.sort()

print "Asteroid name    a (AU)    e        class"
print "-----------------------------------------"
for n in name_list:
    print "%20s %6.4f     %6.4f   %6s" % (n, d[n][0],  d[n][1],  d[n][3])
    #print n + "    " + str(d[n][0]) + "    " + str(d[n][1]) + "  " + str(d[n][3])

#OR

print "Asteroid name    a (AU)    e        class"
print "-----------------------------------------"
for n, a, e, t in sorted(Asteroids):
    print "%20s %6.4f     %6.4f   %6s" % (n, a, e, t)

# semi-major axis

d = dict([(a, (name, e, t, orbit_class[t])) for (name, a, e, t) in Asteroids])
a_list = d.keys()
a_list.sort()
...
print "-----------------------------------------"
for a in a_list:
    print "%20s %6.4f     %6.4f   %6s" % (d[a][0], a, d[a][1], d[a][3])

# OR

def mycmp(val1, val2):

    # compare off of the second element of tuple, rather than 1st as default.
    return cmp(val1[1], val2[1]) 

print "Asteroid name    a (AU)    e        class"
print "-----------------------------------------"
for n, a, e, t in sorted(Asteroids, cmp=mycmp):
    print "%20s %6.4f     %6.4f   %6s" % (n, a, e, t)
...
def switch_elem(val):
    ...
    # rearrange tuple order before doing sort
    return (val[1], val[0], val[2], val[3])

print "Asteroid name    a (AU)    e        class"
print "-----------------------------------------"
for n, a, e, t in sorted(Asteroids, key=switch_elem):
    print "%20s %6.4f     %6.4f   %6s" % (n, a, e, t)
